...
static int __init som_revision(void)
{
	...
	np = of_find_compatible_node(NULL, NULL, "iw,iwg34m-com");
	if (!np) {
...
static void __init carrier_rst(void)
{
	...
	np = of_find_compatible_node(NULL, NULL, "iw,iwg34m-rst");
	if (!np) {
		pr_warn("failed to find iwg34m-rst node\n");
		goto put_node;
	}
	carrier_rst_gpio = of_get_named_gpio(np, "reset-gpio", 0);
	
	if (gpio_is_valid(carrier_rst_gpio) &&
			!gpio_request_one(carrier_rst_gpio, GPIOF_DIR_OUT, "carrier-rst")) {
		gpio_direction_output(carrier_rst_gpio,1);
	}
...
#ifdef CONFIG_IWG34M
	/* IWG34M: Board and BSP info Print */
	print_board_info();
        carrier_rev();
	/* IWG34M: Carrier Board info print */
	carrier_rst();
#endif
	return 0;
